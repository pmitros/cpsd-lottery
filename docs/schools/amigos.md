...
*By MCAS scores, there is still a significant achievement gap -- one of the largest in the district -- although an odd one. In the absolute, low-SES students still perform on-par with or outperform low-SES students in most other CPSD schools on the MCAS; however, the high-SES students just outperform the high-SES students from other schools by a much bigger margin leading to a big gap. The parent I spoke with described the process the school was taking to address the achievement gap in some depth, and said that leading indicators show that the achievement gap is being effectively addressed by the new school administration. Of course, kids stay in the school for a decade, and learning takes time, so it takes time for measures like MCAS scores to catch up to changes in instruction.*

*It is worth pointing out that the measures here -- MCAS scores -- are extremely noisy and inaccurate. Even issues with validity of test data aside (to a large extent, the MCAS measures how much schools teach to the test; a common problem in the district and in Massachusetts), the MCAS data lumps immigrant students from poor countries, students from historic African American communities in housing projects, and children of graduate students into the same "low-SES" category. In practice, these groups are all quite different. Amigos has more immigrants from Spanish-speaking countries and so the demographics of Amigos low-SES students are quite different -- both culturally and educationally -- from students at other schools. And, as the parent I spoke to pointed out, for individual schools, the data sets are often small enough that the error bars are often too large to generalize too much. On the whole, it's difficult to make accurate statements here without qualitative observation.*